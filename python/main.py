...
## Define class Mode which keeps track of the current operating mode and allows for changing it
class Mode:
  ...
  def change(self):
    ...
    volume.reset() # when mode changes, we reset the volume -> useful?
...
## Define class Volume which keeps track of the volume and allows for changing it
class Volume:
...
## Define class Player which handle the VLC player
class Player:
  def __init__(self, path_music):
    ...
    ## fetching music tracks and adding them to VLC
    path_music = os.path.normpath(path_music)
    files_in_path_music = sorted(os.listdir(path_music)) # extract and sort files (since max # should be < 10, simple sorting should work)
    self.tracks_files = list(filter(lambda x: re.search('.*mp3$|.*wav$', x), files_in_path_music)) # only keep file starting with number and ending with mp3
    tracks_paths = [path_music + '/' + s for s in self.tracks_files] # add path to file names
    self.tracks = [instance_vlc.media_new(tracks_paths[i]) for i in range(len(tracks_paths))] # register all tracks to VLC (although only 4 are accessible via push button)

  def play(self, track):
    track_index = track - 1
    "This is the main function that updates the status of the player after a pushed button has been pressed"
    if self.player.get_media() == None or os.path.basename(self.player.get_media().get_mrl()) != self.tracks_files[track_index]:
      # case no track or other track playing -> we start playing the good track
      self.player.stop()
      ...
      if not was_playing and not self.player.is_playing():
        # case no resume possible since track had never started -> play
        self.player.stop() # in case the same track had previously played till end, it needs to be stopped before playing
        self.player.audio_set_volume(volume.get())
        self.player.play()

  def play_track(self, trackname):
    "This function is used to play specific audio file, for clock and system sound"
...
def action_1():
  "This is a button-specific wrapper"
